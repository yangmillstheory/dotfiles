...
vim.api.nvim_create_user_command("YomitanCleanDefinition", function(_)
	local start_pos = StartCommand()
...
vim.api.nvim_create_user_command("YomitanCleanKanji", function(_)
	local start_pos = StartCommand()
...
vim.api.nvim_create_user_command("YomitanSimpleTerm", YomitanSimpleTerm, {
	desc = "Use only the first definition of a Yomitan-mined term.",
})

vim.api.nvim_create_user_command("YomitanCleanJisho", YomitanCleanJisho, {
	desc = "Clean and a copy-pasted Jisho definition.",
})

vim.api.nvim_create_user_command("YomitanPrepareWords", YomitanPrepareWords, {
...
local function _is_jp_char(codepoint)
	...
	return (codepoint >= 0x4E00 and codepoint <= 0x9FFF) -- Kanji
		or (codepoint >= 0x3400 and codepoint <= 0x4DBF) -- Kanji Extension
		or (codepoint >= 0x20000 and codepoint <= 0x2A6DF) -- Kanji Supplement
		or (codepoint >= 0x2A700 and codepoint <= 0x2B73F) -- Kanji Supplement
		or (codepoint >= 0x2B740 and codepoint <= 0x2B81F) -- Kanji Supplement
		or (codepoint >= 0x2B820 and codepoint <= 0x2CEAF) -- Kanji Supplement
		or (codepoint >= 0x2CEB0 and codepoint <= 0x2EBEF) -- Kanji Supplement
...
vim.api.nvim_create_user_command("LookupTerm", function(opts)
	...
	for _, cp in utf8.codes(term) do
		...
		if not _is_jp_char(utf8.codepoint(cp)) then
			print(string.format("%s is not a valid Japanese Kanji because of %s.", term, cp))
			return
		end
	end
	_lookup({
...
keymap("v", "<leader>lk", function()
	...
	vim.defer_fn(function()
		_lookup(kanji_lookups, "", 1000)
	end, 1000)
end, { desc = "Lookup selected Kanji in browser" })

keymap("n", "<leader>lk", ":LookupKanji <c-r><c-a><cr>", { desc = "Lookup Kanji under cursor in browser" })
keymap("n", "<leader>lt", ":LookupTerm <c-r><c-a><cr>", { desc = "Lookup term under cursor in browser" })
keymap("n", "<leader>yd", ":YomitanCleanDefinition<cr>", { desc = "Clean Yomitan HTML definition" })
keymap("n", "<leader>yp", ":YomitanPrepareWords<cr>", { desc = "Prepare Yomitan words for Kanji note" })
keymap("n", "<leader>yk", ":YomitanCleanKanji<cr>", { desc = "Clean Yomitan Kanji HTML" })
keymap("n", "<leader>yj", ":YomitanCleanJisho<cr>", { desc = "Clean Jisho definition" })
